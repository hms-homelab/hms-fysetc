...
static void dns_server_task(void *arg)
{
    ...
    while (true) {
        ...
        // Build DNS response: set QR=1, ANCOUNT=1
        ...
        // Answer: pointer to name, A record, TTL=0, 4 bytes
...
{
    // Use last 4 chars of device name for unique AP SSID
    const char *name = DEVICE_NAME;
    const char *suffix = name + (strlen(name) > 4 ? strlen(name) - 4 : 0);

    char ap_ssid[32];
    snprintf(ap_ssid, sizeof(ap_ssid), "FYSETC-Setup-%s", suffix);
